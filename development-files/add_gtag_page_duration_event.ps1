...
foreach ($file in $htmlFiles) {
    ...
    # Check if the file already has the page_duration event code
    if ($content -match "gtag\('event', 'page_duration'") {
        Write-Host "Skipping (already has page_duration event code): $($file.FullName)"
        continue
    }
