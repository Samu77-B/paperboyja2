...
foreach ($file in $htmlFiles) {
    ...
    # Check if the file already has the file_download event code
    if ($content -match "gtag\('event', 'file_download'") {
        Write-Host "Skipping (already has file_download event code): $($file.FullName)"
        continue
    }
